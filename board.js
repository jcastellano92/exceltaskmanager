...
(function () {
  ...
  async function loadConfigAndDimensions() {
    ...
    // Config → Board columns screen (which can also create these columns):
    //   Color    — per-status colour (unified everywhere)
    //   Order    — board-column order (persists to the workbook)
    //   Complete — Yes/No: this status means done (auto-100%, "Recently completed", closes subtasks)
    //   Blocked  — Yes/No: this status means blocked (at-risk, blocked highlighting)
    //   ShowStat — Yes/No: show this status as a stat tile on My Tasks
    const YES = (v) => /^(y|yes|true|1|x|✓)$/i.test(String(v == null ? "" : v).trim());
    State.statusColors = {};
    State.statusMeta = {};                 // name → { complete, blocked, showStat }
    State.statusHasOrder = false;
    State.statusHasColor = false;
    State.statusHasComplete = false;
    State.statusHasBlocked = false;
    State.statusHasShowStat = false;
    try {
      ...
      State.statusHasComplete = has("Complete");
      State.statusHasBlocked = has("Blocked");
      State.statusHasShowStat = has("ShowStat");
      const meta = statusRows.map((r) => {
        ...
        return { name: String(r[keys[0]] == null ? "" : r[keys[0]]).trim(),
                 ...
                 complete: YES(r.Complete), blocked: YES(r.Blocked), showStat: YES(r.ShowStat) };
      }).filter((m) => m.name);
      if (State.statusHasOrder) {
        meta.sort((a, b) => {
      ...
      meta.forEach((m) => {
        ...
        State.statusMeta[m.name] = {
          complete: State.statusHasComplete ? m.complete : null,
          blocked: State.statusHasBlocked ? m.blocked : null,
          showStat: State.statusHasShowStat ? m.showStat : null
        };
      });
    ...
    try {
      const completeNames = (State.config.Statuses || []).filter((s) => isCompleteStatus(s));
    ...
    return `<article class="go-objective ${open ? "expanded" : ""} ${attention ? "attention-" + attention : ""}" id="goal-${escapeAttr(gid)}" data-goal-card="${escapeAttr(gid)}">
      <div class="go-objective-header">
        <button class="go-objective-main" data-goal-toggle="${escapeAttr(gid)}" type="button">
          ...
          <span class="go-title-block"><strong>${escapeHtml(goal.ShortName || goal.GoalName || gid)}${attention ? `<em class="go-attention-badge ${attention}">${attention === "watch" ? "Watch" : "At risk"}</em>` : ""}</strong>
        ...
        <button class="go-edit-objective" data-edit-goal="${escapeAttr(gid)}" type="button" title="Edit objective">Edit objective</button>
      </div>
  ...
  function showStage2Editor(title, body, onSave, archiveAction) {
    ...
    const attentionSelect = overlay.querySelector("[data-goal-attention]");
    const attentionHelp = overlay.querySelector("[data-attention-help]");
    if (attentionSelect && attentionHelp) {
      const refreshAttentionHelp = () => { attentionHelp.hidden = !["Amber", "Red"].includes(attentionSelect.value); };
      attentionSelect.addEventListener("change", refreshAttentionHelp);
      refreshAttentionHelp();
    }
  ...
  function renderMyDashboard() {
    ...
    const matchMe = (s) => { const v = String(s || "").toLowerCase(); return !!v && (v.includes(meName) || (meEmail && v.includes(meEmail))); };
    const isConnected = (t) => matchMe(t.Owner) || matchMe(t.Contributors);
    const mine = State.tasks.filter(isConnected);
    // Subtasks assigned to me (across everyone's tasks).
    const mySubs = [];
    Object.keys(State.subtasksByParent).forEach((pid) =>
      (State.subtasksByParent[pid] || []).forEach((s) => { if (matchMe(s.Owner)) mySubs.push(Object.assign({ _pid: Number(pid) }, s)); }));

    const q = currentQuarter();
    const todayIso = new Date().toISOString().slice(0, 10);
    const in14Iso = new Date(Date.now() + 14 * 86400000).toISOString().slice(0, 10);
    const openTasks = mine.filter((t) => !isCompleteStatus(t.Status));
    const openSubs = mySubs.filter((s) => String(s.Done).toLowerCase() !== "yes");

    // Stat tiles: one per status flagged "show as stat" (default: every status),
    // counting my connected tasks in the current quarter.
    const qMine = mine.filter((t) => t.Quarter === q);
    let statStatuses = boardColumns().filter(statusShowsStat);
    if (!statStatuses.length) statStatuses = boardColumns();
    const tiles = statStatuses.map((s) => ({ status: s, n: qMine.filter((t) => t.Status === s).length }));

    // Due soon / overdue across my open tasks AND my open subtasks.
    const odue = (d) => { const x = isoDate(d); return x && x < todayIso; };
    const soon = (d) => { const x = isoDate(d); return x && x >= todayIso && x <= in14Iso; };
    const overdueN = openTasks.filter((t) => odue(t.DueDate)).length + openSubs.filter((s) => odue(s.DueDate)).length;
    const dueSoonN = openTasks.filter((t) => soon(t.DueDate)).length + openSubs.filter((s) => soon(s.DueDate)).length;
    const weeks = workWeeksLeft(q);

    // ── Header + context bar ──
    const header = document.createElement("div");
    header.className = "mine-header";
    header.innerHTML = '<h2>My Tasks</h2><span class="mine-who">' + escapeHtml(State.me.name) + '</span>' +
      '<span class="mine-ctx"><b>' + escapeHtml(q) + '</b> · ' + (weeks == null ? "—" : weeks + " work week" + (weeks === 1 ? "" : "s") + " left") + '</span>' +
      '<span class="mine-ctx mine-ctx-soon' + (dueSoonN ? " on" : "") + '">⏳ ' + dueSoonN + ' due ≤14d</span>' +
      (overdueN ? '<span class="mine-ctx mine-ctx-alert">⚠ ' + overdueN + ' overdue</span>' : '');
    root.appendChild(header);

    // ── Per-status stat tiles ──
    const stats = document.createElement("div");
    stats.className = "mine-stats";
    stats.innerHTML = tiles.map((t) =>
      '<div class="mine-stat"><div class="ms-value" style="color:' + statusColor(t.status) + '">' + t.n + '</div>' +
      '<div class="ms-label"><i class="status-dot" style="background:' + statusColor(t.status) + '"></i>' + escapeHtml(t.status) + '</div></div>').join("");
  ...
  // "Work weeks" left in a quarter (business days ÷ 5), or null if unknown.
  function workWeeksLeft(q) {
    ...
    if (endDt < d) return 0;
    let bd = 0, guard = 0;
    while (d <= endDt && guard++ < 400) { const dow = d.getDay(); if (dow >= 1 && dow <= 5) bd++; d = new Date(d.getTime() + 86400000); }
    return Math.max(0, Math.round((bd / 5) * 10) / 10);
  }
  ...
  function recentUpdateRows(taskIdSet, limit) {
    if (!State.allUpdates || !State.allUpdates.length) return [];
    const subParent = {};
    Object.keys(State.subtasksByParent).forEach((pid) =>
      (State.subtasksByParent[pid] || []).forEach((s) => { subParent[Number(s.SubtaskID)] = Number(pid); }));
    return State.allUpdates.map((u) => {
      let tid = null, subText = "";
      if (String(u.ParentType) === "Task") tid = Number(u.ParentID);
      else if (String(u.ParentType) === "Subtask") {
        tid = subParent[Number(u.ParentID)];
        const sub = (State.subtasksByParent[tid] || []).find((s) => Number(s.SubtaskID) === Number(u.ParentID));
        subText = sub ? (sub.Text || "") : "";
      }
      return { u: u, tid: tid, sub: subText };
    }).filter((r) => r.tid != null && taskIdSet.has(r.tid))
      .sort((a, b) => String(b.u.AddedDate).localeCompare(String(a.u.AddedDate)))
      .slice(0, limit || 6);
  }

  function updateLineHtml(r, compact) {
    const t = State.tasks.find((x) => Number(x.TaskID) === Number(r.tid));
    const title = t ? t.Title : "";
    const ws = t ? workstreamName(t.WorkstreamID) : "";
    const tag = r.sub ? '<span class="mu-kind">Subtask</span> ' + escapeHtml(r.sub) : '<span class="mu-kind mu-kind-task">Task</span> ' + escapeHtml(title);
    return '<div class="mine-update" data-upd-task="' + r.tid + '">' +
      '<div class="mu-task">' + tag + '</div>' +
      (r.sub && title ? '<div class="mu-on">on ' + escapeHtml(title) + (ws && ws !== "—" ? ' · ' + escapeHtml(ws) : '') + '</div>' : (ws && ws !== "—" ? '<div class="mu-on">' + escapeHtml(ws) + '</div>' : '')) +
  ...
  function mineSubtaskSection(openSubs) {
    ...
    sec.innerHTML = '<h3>My subtasks (' + openSubs.length + ')</h3>';
    if (!openSubs.length) { sec.innerHTML += '<div class="mine-empty">No open subtasks assigned to you.</div>'; return sec; }
    const todayIso = new Date().toISOString().slice(0, 10);
    openSubs.slice().sort((a, b) => (isoDate(a.DueDate) || "9999-12-31") < (isoDate(b.DueDate) || "9999-12-31") ? -1 : 1).forEach((s) => {
      ...
      cb.addEventListener("change", async () => {
        cb.disabled = true;
        try {
          await window.WsjfData.toggleSubtask(s.SubtaskID, true);
          const arr = State.subtasksByParent[s._pid] || [];
          const it = arr.find((x) => Number(x.SubtaskID) === Number(s.SubtaskID)); if (it) it.Done = "Yes";
          await syncParentPctFromSubtasks(s._pid);
          renderMyDashboard();
  ...
  function mineSection(title, tasks, emptyMsg, opts) {
    opts = opts || {};
    const sec = document.createElement("section");
    sec.className = "mine-section";
    ...
    tasks.forEach((t) => sec.appendChild(mineTaskRow(t, opts)));
    return sec;
  }
  ...
  function mineTaskRow(t, opts) {
    ...
    row.innerHTML =
      (canExpand ? '<button class="mine-caret">' + (expanded ? "▾" : "▸") + '</button>' : (opts.subtasks ? '<span class="mine-caret-sp"></span>' : '')) +
      '<span class="wsjf-pill ' + wsjfPillClass(wsjf) + '">' + wsjf.toFixed(1) + '</span>' +
      '<span class="mine-row-title">' + scheduleChip(t) + escapeHtml(t.Title || "") + '</span>' +
    ...
    if (caret) caret.addEventListener("click", (e) => {
      ...
      if (State.expandedSubtasks.has(id)) State.expandedSubtasks.delete(id); else State.expandedSubtasks.add(id);
      renderMyDashboard();
    });
    wrap.appendChild(row);
  ...
  function workstreamCard(w, opts) {
    ...
    const drillTo = (quarter) => {
      ...
      State.viewMode = "board";   // drill into the board (with filters), not the list
  ...
  function roadmunkMap() {
    try {
      ...
      // Older saved settings used "Operational". Normalize once so exports align
      // with the workbook's Portfolio / Operations grouping without manual cleanup.
      if (String(map.operationalValue || "").trim().toLowerCase() === "operational") map.operationalValue = "Operations";
      return map;
  ...
  function parseCsv(text) {
    ...
    return rows.map((values) => { const item = {}; headers.forEach((header, i) => { item[header] = values[i] || ""; }); return item; });
  }
  ...
  function groupQuarterLabel(start, end) {
    const first = quarterOf(isoDate(start)), last = quarterOf(isoDate(end));
  ...
  function renderConfig() {
    ...
    intro.innerHTML =
      '<h2>Configuration</h2>' +
      '<p class="muted">Add or rename Board columns, Workstreams, Owners and Quarters. Manage objectives and key results on the Goals page. ' +
  ...
  function configSectionRoadmunk() {
    ...
    sec.innerHTML = '<div class="config-section-head"><div><h3>Roadmunk import and export</h3>' +
      '<p class="muted">Configure the Roadmunk fields used to reproduce the Excel roadmap. Workstream values use the Excel workstream name. Roadmap Type exports as Portfolio or Operations.</p></div></div>' +
      '<div class="roadmunk-map-grid">' +
        ...
        '<label>Portfolio / Operations custom field<input id="cfg-rm-type" value="' + escapeAttr(map.typeField) + '"></label>' +
        '<label>Portfolio exported value<input id="cfg-rm-portfolio" value="' + escapeAttr(map.portfolioValue) + '"></label>' +
        '<label>Operations exported value<input id="cfg-rm-operational" value="' + escapeAttr(map.operationalValue) + '"></label>' +
  ...
  function configSectionStatuses() {
    ...
    p.innerHTML = "Your Kanban columns — use whatever names your team uses. Rename cascades to every task; ▲▼ sets order; the swatch sets the colour (used everywhere). " +
      "<b>Complete</b> = this status means done (auto-100%, “Recently completed”, closes subtasks). <b>Blocked</b> = at-risk highlighting. <b>Stat</b> = show this status as a tile on My Tasks. " +
      "All of this is stored in your <b>StatusesTable</b>, so everyone on your team sees the same setup.";
    sec.appendChild(p);

    // If the optional columns are missing, offer to create them so config persists + shares.
    const missing = ["Order", "Color", "Complete", "Blocked", "ShowStat"].filter((c) =>
      !({ Order: State.statusHasOrder, Color: State.statusHasColor, Complete: State.statusHasComplete, Blocked: State.statusHasBlocked, ShowStat: State.statusHasShowStat }[c]));
    if (missing.length) {
      const warn = document.createElement("div");
      warn.className = "config-note config-warn";
      warn.innerHTML = '⚠ These settings won’t save to the workbook until StatusesTable has columns: <b>' + missing.join(", ") + '</b>. ';
      const btn = document.createElement("button");
      btn.className = "btn btn-primary btn-sm";
      btn.textContent = "➕ Add these columns to StatusesTable";
      btn.addEventListener("click", () => configGuard(async () => {
        const n = await window.WsjfData.addTableColumns("StatusesTable", missing);
        toast("Added " + n + " column" + (n === 1 ? "" : "s") + " to StatusesTable.", "info");
        await afterConfigChange();
      }));
    ...
    cols.forEach((val, idx) => {
      ...
      const flags = document.createElement("div"); flags.className = "status-flags";
      flags.innerHTML =
        '<label title="This status means the task is done (auto-100%, closes subtasks, counts as completed)"><input type="checkbox" data-flag="Complete"' + (isCompleteStatus(val) ? " checked" : "") + ' /> Complete</label>' +
        '<label title="This status means the task is blocked (at-risk highlighting)"><input type="checkbox" data-flag="Blocked"' + (isBlockedStatus(val) ? " checked" : "") + ' /> Blocked</label>' +
        '<label title="Show this status as a stat tile at the top of My Tasks"><input type="checkbox" data-flag="ShowStat"' + (statusShowsStat(val) ? " checked" : "") + ' /> Stat</label>';
      ...
      Array.from(flags.querySelectorAll("input[data-flag]")).forEach((cb) => {
        cb.addEventListener("change", () => {
          const col = cb.dataset.flag;
          configGuard(async () => {
            await window.WsjfData.updateStatusRow(val, { [col]: cb.checked ? "Yes" : "No" });
            toast(col + " " + (cb.checked ? "on" : "off") + ' for “' + val + '”.', "info");
            await afterConfigChange();
          });
        });
      });
      ...
      row.appendChild(swatch); row.appendChild(input); row.appendChild(flags); row.appendChild(tag);
  ...
  // Name heuristic used only as a fallback when explicit flags aren't set.
  ...
  // Config-driven status semantics (StatusesTable flags), with a heuristic fallback,
  // so renaming/adding statuses adapts everywhere.
  function isCompleteStatus(name) {
    const m = State.statusMeta && State.statusMeta[name];
    if (m && m.complete != null) return !!m.complete;   // explicit Complete flag
    return statusBucket(name) === "done";
  }
  function isBlockedStatus(name) {
    const m = State.statusMeta && State.statusMeta[name];
    if (m && m.blocked != null) return !!m.blocked;     // explicit Blocked flag
    return statusBucket(name) === "blocked";
  }
  function isReadyStatus(name) { return !isCompleteStatus(name) && !isBlockedStatus(name) && statusBucket(name) === "ready"; }
  // Show this status as a stat tile on My Tasks? Explicit ShowStat flag; if the
  // column isn't set up, show every status by default.
  function statusShowsStat(name) {
    const m = State.statusMeta && State.statusMeta[name];
    if (State.statusHasShowStat && m && m.showStat != null) return !!m.showStat;
    return true;
  }
